...
static inline void local_ctrl_bare(char *buf, int id) {
    snprintf(buf, NAME_MAXLEN, "local_%d", id);
}
static inline void local_ctrl_name(char *buf, int id) {
    snprintf(buf, NAME_MAXLEN, "/net/vm2/dev/name/local/local_%d", id);
}
#define CENTRAL_NAME "central_thread"
#define CENTRAL_ATTACH_POINT "/net/vm1/dev/name/local/central_thread"
